...
- [ ] callback中，存在一个PDU的不同信号超时，要求不同处理的情况，已新建runnable，并且和对应的Event1、Event16映射关联，删除之前的Runnable
- [x] AEB增加和ET的交互 数据类型、interface、port、connector、access都已添加 ✅ 2024-01-25
- [x] ET修改Runnable的周期为10ms ✅ 2024-01-25
- [x] Dcm的MS通信增加MSEvent和Runnale ✅ 2024-01-25
- [ ] DID和Tsyn的交互
- [ ] Dcm_SWC删除DataServices_DataElement_xxxx，删除相应的RTEEvent和Runnable；
- [ ] Dem_SWC增加DataServices_DataElement_xxxx，增加相应的RTEEvent和Runnable；
- [ ] Dem_SWC:
- [ ] DID的B055、56、57、58没有ServerRunnable

1．增加快照数据接口

2．增加读取NvMBlock＿0xB00A和的NvMBlock＿OxB00A接口 FusaMapping:

1．增加向TSyn管理模块调用uint64 R＿Get＿System＿Time（的接口 Mapping:

1．增加向TSyn管理模块调用uint64 R＿Get＿System＿Time（的接口 TSyn:

1．增加uint64 R＿Get＿System＿Time（的接口 Pwr:

1．增加超时回调接口，超时信号为Com＿ADCANFD＿ISTrisSysPwrMd＿0，PDU为CCP＿020ms＿PDU02＿AD DID
